...
from mesh_simple import normalization
...
class Model(nn.Module):
    ...
    def _build_graph(self, inputs, is_training):
        ...
        mesh_edges = core_model.EdgeSet(
            ...
            senders=senders)
        """
        recievers=torch.cat([recievers, world_recievers])
        senders=torch.cat([senders, world_senders])



        edge_set = core_model_att.EdgeSet(
            name='all_edges',
            features=None,
            recievers=torch.cat([recievers, world_recievers]),
            senders=torch.cat([senders, world_senders]),
        ...
        return (core_model_att.MultiGraph(node_features=self._node_normalizer(node_features),
                                        edge_set=edge_set))
